-- Fix duplicate reminders issue by updating the database function to delete existing scheduled reminders first
-- This ensures no duplicates are created even if the function is called multiple times
...
BEGIN
  -- Delete any existing scheduled reminders for this invoice first to prevent duplicates
...
-- Ensure the trigger is disabled (double-check)
